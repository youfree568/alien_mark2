...
class Training:
	...
	def _check_keydown(self, event):
		...
		if event.key == pygame.K_q:
			...
			self._fire_bullet()
		elif event.key == pygame.K_e and not self.game_status:
			self._easy_level()
		elif event.key == pygame.K_h and not self.game_status:
			self._hard_level()
	...
	def check_play_button(self, mouse_pos):
		...
		if self.button.rect1.collidepoint(mouse_pos) and not self.game_status:
			self._easy_level()
		elif self.button.rect2.collidepoint(mouse_pos) and not self.game_status:
			self._hard_level()

	def _easy_level(self):
		# start easy level
		self.game_status = True
		self.settings.initialization_dynamic_settings_easy()

	def _hard_level(self):
		# start hard level
		self.game_status = True
		self.settings.initialization_dynamic_settings_hard()
